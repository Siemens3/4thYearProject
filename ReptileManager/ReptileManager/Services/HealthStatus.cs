...
namespace ReptileManager.Services
{
    ...
    {
        private ReptileContext db;
        public HealthStatus(ReptileContext db)
        {
            this.db = db;
        }
                    ...
                    else if (ReptileType.Born.Value.AddMonths(6) < DateTime.Today)
                    {
                        if (LastFiveWeights.ElementAt(i).Weights > LastFiveWeights.First().Weights)
                        {
                            ...
                            if (WeightLoss >= DangerWeightLoss)
                            {
                                DangerWeight = true;
                                HealthStatus += 2;
                                weightLossMessage = "ID:" + id + " has lost " + WeightLoss + ". This is very seriours. Helpful links:" + UriLeoHelp_001.ToString();
                                report.Add(weightLossMessage);
                            }
                            else if (WeightLoss < MinorWeightLoss)
                            {
                                MinorWeight = true;
                                HealthStatus += 1;
                                weightLossMessage = "ID:" + id + " has lost " + WeightLoss + ". This reptile requires attention. Helpful links:" + UriLeoHelp_001.ToString();
                                report.Add(weightLossMessage);
                            }
                        }
                    }
